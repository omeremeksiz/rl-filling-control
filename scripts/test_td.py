...
CONFIG_PATH = os.path.join("configs", "td_test.yaml")
...
def main() -> None:
    ...
    try:
        for ep in range(episodes):
            ...
            logger.info(f"Dispatching switching point: {experienced_switch_point}")

            modbus.send_switch_point(float(experienced_switch_point))
            ...
            best_switch_point = experienced_switch_point
            for state in sorted(state_to_best.keys()):
                if state_to_best[state][0] == -1:
                    best_switch_point = state
                    break

            next_switch_point, explored_choice = _pick_next_switch_point(
                best_switch_point,
                known_switch_points,
                epsilon,
                step_weights,
            ...
            persist_episode(
                ...
                switch_point=int(experienced_switch_point),
            ...
            episode_records.append(
                {
                    "episode_num": ep,
                    "experienced_switching_point": experienced_switch_point,
    ...
    sp_bounds = (min(known_switch_points), max(known_switch_points)) if known_switch_points else (0, 1)
    plot_switching_trajectory_with_exploration(
        traj_ep,
        model_selected_list,
        explored_list,
    ...
    if episode_records:
        ...
        logger.info("Saved TD Q-value updates to %s", excel_output_path)
