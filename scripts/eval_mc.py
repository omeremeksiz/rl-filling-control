...
def run_experiment(
    ...
    for episode in range(episodes):
        ...
        for state, action, reward in reversed(trajectory):
            ...
            q_table[(state, action)] = q_sa + (1 / n) * (G - q_sa) # adaptive learning rate
...
def main() -> None:
    ...
    for name, summaries in results_by_name.items():
        ...
        all_keys = sorted({key for table in q_tables for key in table.keys()})
        median_table: Dict[Tuple[int, int], float] = {}
        for key in all_keys:
            values = [table.get(key) for table in q_tables if key in table]
            values = [value for value in values if value is not None and np.isfinite(value)]
            if values:
                median_table[key] = float(np.median(values))
        qvalue_data[name] = median_table
        ...
        best_sp_map[name] = int(round(float(np.median(best_values)))) if best_values else None

    plot_format = _resolve_plot_format(cfg.get("plot_format"))

    comparison_paths = {
        "switching": os.path.join(output_dir, f"mc_eval_switching_trajectory.{plot_format}"),
        "qvalues": os.path.join(output_dir, f"mc_eval_qvalue_comparison.{plot_format}"),
    }
