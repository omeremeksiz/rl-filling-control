...
def load_config() -> Dict[str, Any]:
    with open(os.path.join("configs", "q_test.yaml"), "r", encoding="utf-8") as f:
...
def main() -> None:
    ...
    # Online Q-learning with state as observed weight
    ...
    try:
        for ep in range(episodes):
            ...
            final_state = current_switch_point
            ...
            for _ in range(max_steps):
                ...
                s_t = int(max(0.0, float(np.mean(xs)) if xs else 0.0))
                final_state = s_t
                observed_states.add(s_t)

                a_t = 1 if s_t < current_switch_point else -1
                r = calc_reward_standard(s_t, safe_min=safe_min, safe_max=safe_max)

                q_fast = q_map.get((s_t, 1), 0.0)
                q_slow = q_map.get((s_t, -1), 0.0)
                best_next = max(q_fast, q_slow)
                q_sa = q_map.get((s_t, a_t), 0.0)
                td_target = r + gamma * best_next
                q_map[(s_t, a_t)] = q_sa + alpha * (td_target - q_sa)
            ...
            store_sequence = core_sequence if core_sequence else (weight_trace if weight_trace else [final_state])
            if episode_length_db <= 0:
                episode_length_db = len(store_sequence)

            state_to_best: Dict[int, Tuple[int, float]] = {}
            for (state, action), value in q_map.items():
            ...
            persist_episode(
                ...
                final_weight=int(final_state),
                safe_min=safe_min,
                safe_max=safe_max,
                q_values_snapshot=q_map.copy(),
                meta=meta,
            )
    ...
    if q_map:
        plot_qvalue_vs_state_from_pair_table(q_map, paths['qvalue_vs_state_path'])
    plot_switching_trajectory_with_exploration(
        episode_indices,
        model_selected_list,
        explored_list,
