...
class QLearningAgent:
    ...
    def select_action(self, current_switch_point: Optional[int] = None) -> int:
        """
        Select next action using epsilon-greedy policy with step-based exploration.
        ...
        if random.random() < self.exploration_rate:
            # Exploration: move +1 to +5 steps from best action with decreasing probabilities
            best_switch_point = self._get_best_switch_point()
            
            # Get available switch points sorted by distance from best action
            available_points = sorted(self.available_switch_points, 
                                   key=lambda x: abs(x - best_switch_point))
            
            # Find the best action index in available points
            best_index = available_points.index(best_switch_point)
            
            # Try each exploration step with decreasing probability
            for step, prob in zip(EXPLORATION_STEPS, EXPLORATION_PROBABILITIES):
                if random.random() < prob:
                    # Try to move +step from best action
                    target_index = best_index + step
                    if target_index < len(available_points):
                        return available_points[target_index]
            
            # If no exploration step was selected, return best action
            return best_switch_point
        else:
            # Exploitation: choose switch point with highest Q-value
