...
from config import (
    ...
    DEFAULT_RANDOM_SEED, DEFAULT_STARTING_SWITCH_POINT, DEFAULT_PENALTY_MULTIPLIER
)
...
def main():
    ...
    # Initialize the system with all parameters from config
    system = FillingControlSystem()
    
    # Train the agent with all parameters from config
    training_history = system.train(
        num_episodes=DEFAULT_TRAINING_EPISODES,
        initial_switch_point=DEFAULT_STARTING_SWITCH_POINT
    )
    
    # Analyze results with all parameters from config
