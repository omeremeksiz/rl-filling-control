...
class TrainingLogger:
    ...
    def log_training_summary(self, training_history: List[Dict]) -> None:
        ...
        # Get Q-table to find best switch point and Q-value
        q_table = {}
        for episode in training_history:
            switch_point = episode.get('switch_point')
            if switch_point is not None:
                q_table[switch_point] = episode.get('q_value', 0.0)
        
        if q_table:
            best_switch_point = max(q_table, key=q_table.get)
            best_q_value = q_table[best_switch_point]
        else:
            best_switch_point = "N/A"
            best_q_value = 0.0
        ...
        self.logger.info(f"Best Switch Point: {best_switch_point}")
        self.logger.info(f"Best Q-Value: {best_q_value:.2f}")
        self.logger.info("="*50)
    ...
    def _create_qvalue_vs_state_plot(self, q_table: Dict[int, float], file_path: str) -> None:
    ...
    def _create_trajectory_plot(self, training_history: list, file_path: str, line_width=1.5) -> None:
        ...
        plt.title("Next Switch Point Trajectory", fontsize=16, weight='bold')
