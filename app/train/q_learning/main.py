...
def main():
    ...
    q_value_handler.write_to_text(config.ql_qvalue_path)
    ...
    best_action_handler.write_to_text(config.ql_best_actions_path)

    # Format the single Q-value file to HTML
    file_handler_module.FileHandler().format_and_write_table(config.ql_qvalue_path, config.ql_qvalue_html_path, column_names=['Weight', 'Action', 'Count', 'Q Value'])

    # Format the single best action file to HTML
    file_handler_module.FileHandler().format_and_write_table(config.ql_best_actions_path, config.ql_best_actions_html_path, column_names=['Weight', 'Best Action', 'Count', 'Q Value'])
